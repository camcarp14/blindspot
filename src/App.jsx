...
const DAILY_QUOTA = 5000 // eBay's default Browse API limit; raise here if you've requested more
const todayKey = () => `blindspot.budget.${new Date().toISOString().slice(0, 10)}`
...
export default function App() {
  ...
  const [dealsConfigured, setDealsConfigured] = useState(false)
  ...
  const [todayCalls, setTodayCalls] = useState(() => loadJSON(todayKey(), 0))

  useEffect(() => {
    api
      .health()
      .then((h) => setStatus((s) => ({ ...s, token: h.ok ? 'live' : 'error', tokenError: h.error })))
      .catch(() => setStatus((s) => ({ ...s, token: 'error' })))
    api
      .dealsStatus()
      .then((d) => setDealsConfigured(!!d.configured))
      .catch(() => setDealsConfigured(false))
  }, [])
  ...
  useEffect(() => {
    localStorage.setItem(todayKey(), JSON.stringify(todayCalls))
  }, [todayCalls])
  ...
  const scan = async () => {
    ...
    try {
      ...
      setTodayCalls((n) => n + (data.apiCalls || 0))
  ...
  const saveDeal = async (deal) => {
    try {
      ...
      // silent — button just won't flip to "Saved"; not worth a banner for this
    }
  }
  ...
  const econ = { ...DEFAULT_ECON, shipEstimate: Number(cfg.shipEstimate) || 15 }
    ...
    <div className="shell">
      <header className="topbar">
        ...
        <div className="statusbar mono">
          ...
          <span>{status.apiCalls} this session</span>
          <span className="sep">·</span>
          <span className={todayCalls > DAILY_QUOTA * 0.8 ? 'budget-warn' : ''}>
            {todayCalls}/{DAILY_QUOTA} today
          </span>
        </div>
      </header>
              ...
              <div className="breakdown-list">
                {queryStats.map((q, i) => (
                  <span key={i} className={`breakdown-pill ${q.commonTerm ? 'breakdown-flag' : ''}`}>
                    ...
                    {q.skipped ? 'skipped (known term)' : `${q.raw} hits`}
          ...
          {!results && !scanning && (
            <div className="empty">
              ...
              <p>
                ...
                misspelled brand variants that regular searchers never see — and now screens
                out real words that only look like typos.
              </p>
            </div>
          )}
          ...
          {visible &&
            visible.map((deal, i) => (
              <DealCard
                ...
                dealsConfigured={dealsConfigured}
                econ={econ}
      ...
      <footer className="foot">
        Scanner + alerts only — bids are placed by you, on eBay. Press "S" to scan from anywhere.
      </footer>
