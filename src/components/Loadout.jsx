...
export default function Loadout({ cfg, setCfg, onScan, scanning, open, onToggle, presetLabel }) {
  ...
  const loadPreset = (p) => {
    setCfg((c) => ({
      ...
      typoExclude: '',
      excludeKeywords: '',
      conditionIds: p.conditionIds.join(','),
  ...
  return (
    <aside className="loadout">
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        aria-controls="loadout-body"
      >
        <span>Loadout{presetLabel ? ` · ${presetLabel}` : ''}</span>
        <span className="chevron" aria-hidden="true">{open ? '▴' : '▾'}</span>
      </button>

      <div id="loadout-body" className={`loadout-body ${open ? '' : 'collapsed'}`}>
        <div className="preset-tier">
          <span className="preset-tier-label">Boutique — high asymmetry, low volume</span>
          <div className="preset-grid">
            {PRESETS.filter((p) => p.tier === 'boutique').map((p) => (
              <button
                key={p.id}
        ...
        <div className="preset-tier">
          <span className="preset-tier-label">Scale — fewer pickers, higher $ per catch</span>
          <div className="preset-grid">
            {PRESETS.filter((p) => p.tier === 'scale').map((p) => (
              <button
                key={p.id}
                className={`preset ${cfg.presetId === p.id ? 'preset-active' : ''}`}
                onClick={() => loadPreset(p)}
        ...
        <label className="field">
          <span>Typo exclude — real words that look like typos</span>
          <input
            className="input"
            value={cfg.typoExclude}
            onChange={set('typoExclude')}
            placeholder="nikor is already excluded by default"
          />
        </label>

        <label className="field">
          <span>Exclude keywords in title</span>
          <input
            className="input"
            value={cfg.excludeKeywords}
            onChange={set('excludeKeywords')}
            placeholder="repro, replica, case only"
          />
        </label>
        ...
        <label className="field">
          <span>Condition IDs — 1000 New · 3000 Used · 7000 For parts</span>
          <input className="input" value={cfg.conditionIds} onChange={set('conditionIds')} placeholder="optional" />
        </label>
